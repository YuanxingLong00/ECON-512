...
% Estimate theta
F= @(theta) (exp(theta(1)*X1+theta(2)*X2+theta(3)*X3+theta(4)*X4)./(ones(N,1)+exp(theta(1)*X1+theta(2)*X2+theta(3)*X3+theta(4)*X4)));
...
Xmu1xp= @(p)( sum(X.* repmat( (mu1x.*K((pXhat-p*ones(N,1))/h)) ,1,4), 1) ./ sum( K((pXhat-p*ones(N,1))/h) ) );
Xmu0xp= @(p)( sum(X.*repmat( (mu0x.*K((pXhat-p*ones(N,1))/h)) ,1,4), 1) ./ sum( K((pXhat-p*ones(N,1))/h) ) );
Xp= @(p)( sum(X.*repmat( K((pXhat-p*ones(N,1))/h) ,1,4), 1) ./ sum( K((pXhat-p*ones(N,1))/h) ) );
...
cov1p= @(p)( Xmu1xp(p)- Xp(p).*repmat(mu1xp(p),1,4) );
cov0p= @(p)( Xmu0xp(p)- Xp(p).*repmat(mu0xp(p),1,4) );
